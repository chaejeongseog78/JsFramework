...
(function ($) {
	...
	$(document).ready(function () {
		var listener = function () {
			...
			$(document).on(
				"click",
				".btn_cal_today, .btn_cal_yesterday, .btn_cal_week, .btn_cal_month, .btn_cal_month1, .btn_cal_month2, .btn_cal_month3, .btn_cal_total",
				function (e) {
					setTimeout(function () {
						getList(1);
					}, 500);
			...
			$(document).on("click", "#prtPDF", function (e) {
				e.preventDefault();
				e.stopPropagation();
				e.returnValue = false;
				...
				if (chkLength > 0) {
					...
					document.getElementById("TmpFrame").src =
						"/gJSboardExec.php?action=prtPDF&chkdats=" +
						encodeURIComponent(chkdats);
			...
			$(document).on("click", "#check_all", function (e) {
				...
				if ($(this).is(":checked") === true) {
					$('input:checkbox[name="check[]"]:not(checked)').prop(
						"checked",
						true
					);
				} else {
			...
			$.ajax({
				...
				data: {
					...
					param: encodeURIComponent(
						JSON.stringify($("#form_reg").serializeJSON())
					),
				},
				success: function (result) {
					gHideLoadingBar();
					if (isset(result) && result["MSG"] == "OK") {
						if (goUrl !== null) goTo(goUrl);
		...
		var getList = function (page) {
			...
			currPage = page;
			$.ajax({
				url: window.$_DBCONN_HTTP_URL + "/ajax/exec/gJSboardExec.php",
				cache: true,
				type: "post",
				dataType: "jsonp",
				crossDomain: true,
				data: {
					action: "gGetList",
					page: page,
					numperpage: $("#limit_select option:selected").val(),
		...
		var listUpDisplay = function (data, page) {
			...
			for (; loop < len; loop++) {
				divData +=
					"<tr>" +
					'<td class="tdlC"><input type="checkbox" name="check[]" class="idx" value="' +
					obj[loop]["idx"] +
					'"><br>' +
					(currPage * currPerPage + loop + 1 - currPerPage) +
					"</td>" +
					'<td class="tdlC">' +
					obj[loop]["subject"] +
					"</td>" +
					...
					obj[loop]["signdate"] +
					"</td>" +
					'<td class="tdlC"><button type="button" data-idx=' +
					obj[loop]["idx"] +
					' class="btn btn-primary btn-xs gModify">수정</button></td>' +
					"</tr>";
			}
		...
		var disPageing = function (page, data) {
			...
			if (prevPage != 1)
				divData +=
					'<a class="launch_page_select" pageno="' +
					(prevPage - 1) +
					'" style="cursor:pointer"> Prev </a>';
			for (var i = prevPage; i <= nextPage; i++) {
				if (i == page)
					divData +=
						'<a class="current launch_page_select" pageno="' +
						i +
						'"><strong>' +
						i +
						"</strong></a>";
		...
		var getUnZipDat = function (data) {
			...
			for (var i = 0; i < data["res_result"].length; i++) {
				...
				if (!empty(data["res_result"][i]["MEM"]["MBI_9001"])) {
					...
					data["res_result"][i]["MEM"]["MBI_9001"] = unzip(
						data["res_result"][i]["MEM"]["MBI_9001"],
						idx
					);
					//trace(JSON.parse(data['res_result'][i]['MEM']['MBI_9001']));
				}
				if (!empty(data["res_result"][i]["MEM"]["MBI_9010"])) {
					data["res_result"][i]["MEM"]["MBI_9010"] = unzip(
						data["res_result"][i]["MEM"]["MBI_9010"],
						idx
					);
				}
				if (!empty(data["res_result"][i]["MEM"]["MBI_9020"])) {
					data["res_result"][i]["MEM"]["MBI_9020"] = unzip(
						data["res_result"][i]["MEM"]["MBI_9020"],
						idx
					);
				}
				if (!empty(data["res_result"][i]["MEM"]["MBI_9030"])) {
					data["res_result"][i]["MEM"]["MBI_9030"] = unzip(
						data["res_result"][i]["MEM"]["MBI_9030"],
						idx
					);
				}

				if (!empty(data["res_result"][i]["Youl"])) {
					data["res_result"][i]["Youl"] = unzip(
						data["res_result"][i]["Youl"],
						idx
					);
				}
				if (!empty(data["res_result"][i]["BXI_0030"])) {
					data["res_result"][i]["BXI_0030"] = unzip(
						data["res_result"][i]["BXI_0030"],
						idx
					);
				}
				if (!empty(data["res_result"][i]["BXI_0031"])) {
					data["res_result"][i]["BXI_0031"] = unzip(
						data["res_result"][i]["BXI_0031"],
						idx
					);
				}
				if (!empty(data["res_result"][i]["BXI_0032"])) {
					data["res_result"][i]["BXI_0032"] = unzip(
						data["res_result"][i]["BXI_0032"],
						idx
					);
				}
				if (!empty(data["res_result"][i]["BXI_0033"])) {
					data["res_result"][i]["BXI_0033"] = unzip(
						data["res_result"][i]["BXI_0033"],
						idx
					);
				}
				if (!empty(data["res_result"][i]["BXI_0034"])) {
					data["res_result"][i]["BXI_0034"] = unzip(
						data["res_result"][i]["BXI_0034"],
						idx
					);
				}
			}
		...
		var saveJSModalNotDrag = function (html, w, h, uid, inputname) {
			...
			$(".savemodalnotdrag .bg_fixed .sectionview").css({
				width: w,
				height: h,
			});
		...
		var resizeJSModal = function () {
			$(".savemodal .bg_fixed .sectionview").css({
				height: $("#savemodal_contents").height() + 60,
			});
			$(".savemodalnotdrag .bg_fixed .sectionview").css({
				height: $("#savemodalnotdragContents").height() + 60,
			});
			$(".savemodal_2 .bg_fixed .sectionview").css({
				height: $("#savemodal_contents_2").height() + 60,
			});
			$(".savemodal_3 .bg_fixed .sectionview").css({
				height: $("#savemodal_contents_3").height() + 60,
			});
		};
